...
namespace Galchonok
{
    public class GameAold1 : PageInit
    {
        ...
        private void Start()
        {
            if (_right) _librarion = new LibrarionOne(new GameAsettings() );
        ...
        private void Next(bool next)
        {
            ...
            _sequence.Insert( 0, _questionField.DOText(question, duration).SetEase(_ease) );
            _sequence.Insert( 0, _questionField.DOFade(0.1f, duration/2).SetEase(Ease.Linear) );
            _sequence.Append( _questionField.DOFade(1f, duration/2).SetEase(Ease.Linear) );
        ...
        private bool[] SetClickButton(int count, List<int> list)
        {
            ...
            foreach (var child in list)  numbers[child] = true;
            return numbers;
        }
